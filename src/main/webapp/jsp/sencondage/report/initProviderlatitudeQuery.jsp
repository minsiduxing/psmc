...
<script type="text/javascript">
	function exportInfo() {
		var url = '<c:url value="/sencondage/report/electricDataDig.do?method=exportProviderlatitudeList"/>';
	...
	function queryInfo() {
		$("#queryForm")
				.attr(
						"action",
						'<c:url value="/sencondage/report/electricDataDig.do?method=queryProviderlatitudeReportList"/>');
		$("#queryForm").submit();
				...
				<table class="table">
					<tr>
						...
						<td class="inputTd"><input type="text"
							onfocus="WdatePicker({dateFmt: 'yyyy-MM',isShowClear:false})" id="fileInputDateSq"
							name="fileInputDateSq"
							value="<fmt:formatDate value="${curDate}" pattern="yyyy-MM"/>" 
							readonly />

							至 <input type="text" onfocus="WdatePicker({dateFmt: 'yyyy-MM',isShowClear:false})"
